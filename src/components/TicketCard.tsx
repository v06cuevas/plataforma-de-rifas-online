...
export const TicketCard: React.FC<TicketCardProps> = ({ ticket, onViewRaffle }) => {
  ...
  return <>
    <article id={`ticket-${ticket.id}`} className="relative overflow-hidden rounded-2xl border-2 border-slate-400 bg-gradient-to-br from-slate-200 via-slate-100 to-slate-300 shadow-md">
      ...
      <div className="flex flex-col gap-2 border-t border-slate-400/80 bg-slate-300/70 px-3 py-2.5 sm:flex-row sm:items-center sm:justify-between sm:px-4"><span className="text-xs font-medium text-slate-800">{ticket.isBonusTicket ? 'Boleto de regalo · RD$ 0' : <>Monto pagado: <strong className="font-mono">RD$ {ticket.pricePaid.toLocaleString()}</strong></>}</span><div className="flex flex-col gap-2 w-full sm:flex-row sm:w-auto">{onViewRaffle && <button type="button" onClick={() => onViewRaffle(ticket.raffleId)} className="inline-flex flex-1 items-center justify-center gap-1 rounded-lg border border-emerald-700 bg-emerald-100 px-3 py-1.5 text-[11px] font-black text-emerald-900 shadow-sm hover:bg-emerald-200 sm:flex-none"><ExternalLink className="h-3.5 w-3.5" /> Ver Rifa</button>}<button type="button" onClick={() => setShowDetailsModal(true)} className="inline-flex flex-1 items-center justify-center gap-1 rounded-lg border border-blue-700 bg-blue-100 px-3 py-1.5 text-[11px] font-black text-blue-900 shadow-sm hover:bg-blue-200 sm:flex-none"><Eye className="h-3.5 w-3.5" /> Ver Certificado</button><button type="button" onClick={async () => { setIsDownloadingPdf(true); await generateTicketPdf(ticket); setIsDownloadingPdf(false); }} disabled={isDownloadingPdf} className="inline-flex flex-1 items-center justify-center gap-1 rounded-lg border border-purple-700 bg-purple-100 px-3 py-1.5 text-[11px] font-black text-purple-900 shadow-sm hover:bg-purple-200 disabled:opacity-50 disabled:cursor-not-allowed sm:flex-none"><Download className="h-3.5 w-3.5" /> {isDownloadingPdf ? 'Descargando...' : 'Descargar PDF'}</button></div></div>
    </article>
