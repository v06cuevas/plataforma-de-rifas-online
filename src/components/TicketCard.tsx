import React, { useState, useRef, useEffect } from 'react'; // olvin: + useRef, useEffect
...
// olvin: nuevas props para soportar la auto-descarga al llegar por QR
...
export const TicketCard: React.FC<TicketCardProps> = ({ ticket, onViewRaffle, autoDownload, onAutoDownloadHandled }) => {
  ...
  const cardRef = useRef<HTMLElement>(null); // olvin: ref al boleto real para capturarlo tal cual
  const ticketDisplayId = ticket.id.split('-').pop()?.replace(/\D/g, '') || ticket.id.replace(/\D/g, '');

  // olvin: función de descarga extraída para reutilizarla en el click manual y en el autoDownload
  ...
  // olvin: cuadro de confirmación NATIVO del sistema (window.confirm se renderiza
  // con el estilo propio de iOS/Android/escritorio, ya no un modal hecho a mano).
  const handleDownloadClick = () => {
    const wantsDownload = window.confirm('¿Deseas descargar la imagen de tu boleto en alta calidad?');
    if (wantsDownload) runDownload();
  };

  // olvin: cuando se llega a este boleto escaneando su código QR, preguntamos
  // automáticamente si se desea descargar (una sola vez), con la misma
  // ventana nativa del sistema.
  useEffect(() => {
  ...
  return <>
    <article ref={cardRef} id={`ticket-${ticket.id}`} className="relative overflow-hidden rounded-2xl border-2 border-slate-400 bg-gradient-to-br from-slate-200 via-slate-100 to-slate-300 shadow-md">{/* olvin: ref agregado para poder capturar este boleto tal cual con html2canvas */}
